...
def ping():
    ...
    last_ping_time = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
    return {
        "message": "pong",
...
# Global variable for last ping time
last_ping_time = "Never"

def auto_ping():
    """Auto-ping function to keep the app awake"""
    global last_ping_time
    while True:
        try:
            # Get the app URL from environment variable or use default
            app_url = os.environ.get('APP_URL', 'https://genetic-tally-syapa-king.koyeb.app')
            ...
            for endpoint in endpoints:
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"✅ Auto-ping successful: {endpoint}")
                    else:
                        logger.warning(f"⚠️ Auto-ping warning: {endpoint} - Status {response.status_code}")
                except Exception as e:
                    logger.error(f"❌ Auto-ping failed: {endpoint} - {str(e)}")
            
            last_ping_time = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
            logger.info(f"🔄 Auto-ping completed at {last_ping_time}")
            
        except Exception as e:
            logger.error(f"❌ Auto-ping error: {str(e)}")
...
def main() -> None:
    logger.info("🚀 Starting Syapa Bot with Auto-Ping System...")
    
    # Start Auto-Ping thread
    ping_thread = threading.Thread(target=auto_ping)
    ping_thread.daemon = True
    ping_thread.start()
    logger.info("✅ Auto-ping system started (every 5 minutes)")
    ...
    logger.info("🔄 Auto-ping active: Every 5 minutes")
    logger.info("👑 Owner: SYAPA KING")
    logger.info("🔧 Bot is now running and will stay awake!")
